...
//         将用户信息字符串转换为 JSON 对象
        ...
        try {
            ...
            // 将 JSON 对象转换为 JwtUser 对象
            JwtUser user = objectMapper.treeToValue(userInfoNode, JwtUser.class);
            // 将用户信息放入 ThreadLocal 中
            AuthUtil.setUser(user);
        } catch (IOException e) {
            log.error("UserInfoInterceptor.preHandle: {}", e.getMessage(), e);
        }
        return true;
    }
}
